...
var getNewlineReport = function getNewlineReport(rejectedTwigs) {
  return "" + rejectedTwigs.join("\n");
};

var ReportUtil = {
  getNewlineReport: getNewlineReport
};
...
var purify = function purify(searchThrough, css, options, callback) {
    ...
    if (options.rejected && selectorFilter.rejectedSelectors.length) {
        if (options.rejectedOutput) {
            var report = ReportUtil.getNewlineReport(selectorFilter.rejectedSelectors);
            fs.writeFile(options.rejectedReport, report, function (err) {
                if (err) return err;
            });
        } else {
    ...
    if (options.output) {
        ...
        var _report = ReportUtil.getNewlineReport(selectorFilter.rejectedSelectors);
        return callback ? callback({ source: source, report: _report }) : source;
    } else {
